...
void CTranspositionTable::CalculateInitHashKey(BYTE CurPosition[8][8])
{
	int j,k,nChessType;
	m_HashKey32 = 0;
	m_HashKey32 = 0;
...
void CTranspositionTable::Hash_MakeMove(CHESSMOVE* move, BYTE CurPosition[8][8])
{
	BYTE nToID, nFromID;
...
void CTranspositionTable::Hash_UnMakeMove(CHESSMOVE* move, BYTE nChessID, BYTE CurPosition[8][8])
{
	BYTE nToID;
	nToID = CurPosition[move->To.y][move->To.x];
